...
Public Class clsCashier
    ...
    Public Function save()
        Try
            ConnectDatabase()
            Dim query = "INSERT INTO cashier (user_id, username, password, is_active) " &
    ...
    Public Function edit()
        Try
            ConnectDatabase()
            Dim query = "UPDATE cashier SET password=@password, is_active=@is_active WHERE user_id=@user_id"
            cm = New MySqlCommand(query, con)
            cm.Parameters.AddWithValue("@user_id", Id)
            cm.Parameters.AddWithValue("@password", Password)
            cm.Parameters.AddWithValue("@is_active", Active)
    ...
    Public Sub loadRecord()
        ...
        ConnectDatabase()
        Dim query = "SELECT user.user_id,  user_surname, user_gname, user_mi, user_suffix, cashier.username, cashier.is_active FROM user " &
        ...
        While dr.Read
            i += 1
            frmUser.DataGridView2.Rows.Add(dr.Item("user_id").ToString, i, dr.Item("user_gname").ToString, dr.Item("user_mi").ToString, dr.Item("user_surname").ToString, dr.Item("user_suffix").ToString, dr.Item("username").ToString, dr.Item("is_active"), "EDIT", "DELETE")
        End While
        dr.Close()
        DisconnectDatabase()
    ...
    Public Function loadPassword(_username As String)
        ConnectDatabase()
        Dim query = "SELECT password FROM cashier WHERE username=@username"
        cm = New MySqlCommand(query, con)
